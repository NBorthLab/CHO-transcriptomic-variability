...
monte_carlo_int_NB <- function(dat, mu, gamma, phi, gene.subset.n) {
  ...
  for (i in 1:nrow(dat)) {
    ...
    if (!is.null(gene.subset.n) & is.numeric(gene.subset.n) & length(gene.subset.n) == 1) {
      if (i == 1) {
        cat(sprintf("Using %s random genes for Monte Carlo integration\n", gene.subset.n))
      }
      ...
      if (i == 1) {
        cat("Using all genes for Monte Carlo integration; the function runs very slow for large number of genes\n")
      }
      G_sub <- nrow(dat) - 1
    }
...
NPM_lm_DEpipe <- function(cts, batch, group, alpha.unadj, alpha.fdr) {
  ...
  de_called <- list(
    unadj = rownames(cts)[pval_seq < alpha.unadj], fdr = rownames(cts)[padj_seq < alpha.fdr],
