...
function(x, geneSetID, title = "", color = "green", base_size = 11,
  ...
  p.res <- p.res + ylab("Running Enrichment Score") + theme(
  ...
  p.pos <- p + geom_segment(data = df2, aes_(
    x = ~x, xend = ~x,
    y = ~y, yend = 0
  ), color = "grey")
  ...
  if (pvalue_table) {
    ...
    p.res <- p.res + theme(legend.position = "none") + annotation_custom(tp,
      xmin = quantile(p.res$data$x, 0.5), xmax = quantile(
        p.res$data$x,
        0.95
      ), ymin = quantile(
